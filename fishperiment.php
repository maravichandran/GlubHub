...
$query = "SELECT fisherman_id FROM fishes NATURAL JOIN fishermen WHERE first_name='" . $_POST['firstname'] . "' OR last_name='" . $_POST['lastname'] . "'";
$result = mysql_query($query);
$row = mysql_fetch_row($result);
$fishermanID = $row[0];
$query = "SELECT fish_image FROM images NATURAL JOIN fishes NATURAL JOIN fishermen WHERE fisherman_id='" . $fishermanID . "'";
$image_info_table = mysql_query($query);
// Call a function defined later in this file, with two arguments
display_table($fishermanID, $image_info_table, $dbname);
...
echo '<form action="fishperiment.php" method="post"><pre>'; 
 ...
 * @return null
 */
function display_table($fishermanID, $image_info_table, $dbname)
{
	...
	if ($image_info_table)
	{
		...
		for ($count = 0; $count < mysql_num_rows($image_info_table); $count++)
		{
			...
			echo "<TD><img src='http://$domain/~hanasm6s/aprilla/$image_name' style='width:300px' /></TD>";
			if ($count % 6 == 5)
			{
				echo "</TR>";
				$closed_tr = 1;
